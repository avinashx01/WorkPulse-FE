...
const SubMenu: ForwardRefRenderFunction<HTMLLIElement, SubMenuProps> = (props, ref) => {
  ...
  const [localSubMenuOpen, setLocalSubMenuOpen] = useState<boolean>(defaultOpen || false);


  const isContextSubMenuOpen = openSubmenu?.some((item: OpenSubmenu) => item.id === id) ?? false;
  const normalOpenState = isContextSubMenuOpen || localSubMenuOpen;

  const isSubMenuOpen: boolean = forceExpand === true ? true : normalOpenState;

  const isForceExpandProvided = Object.prototype.hasOwnProperty.call(props, 'forceExpand');
  ...
  const handleSlideToggle = (): void => {
    ...
    // Allow toggling regardless of forceExpand
    // This gives priority to user interaction
    toggleOpenSubmenu?.({ level, label, active, id });
    
    // Calculate the new state - inverse of current state
    const newState = !normalOpenState;
    
    // Update local state
    setLocalSubMenuOpen(newState);
    ...
    useEffect(() => {
      if (isForceExpandProvided && onOpenChange) {
        onOpenChange(forceExpand === true);  // Ensure we pass a boolean
      }
    }, [forceExpand, onOpenChange, isForceExpandProvided]);
  ...
  useEffect(() => {
    // When the search query changes, check if the submenu should be open.
    if (confirmUrlInChildren(children, pathname) || defaultOpen) {
      // Push the submenu into the open state if not already present.
      if (!openSubmenusRef?.current.find(sub => sub.id === id)) {
        openSubmenusRef?.current.push({ level, label, active: confirmUrlInChildren(children, pathname), id });
      }
    } else {
      // Optionally, remove it from the open state if needed.
    }
  }, [defaultOpen, pathname, children, id, label, level, openSubmenusRef]);
